...
- Relevant logs (server, client, build output)
- Minimal repro steps
...
- Trace across service boundaries, not just within a single file:
  - request -> middleware -> handler -> service -> data layer -> response
- If the bug is in deprecated or superseded code, confirm whether the correct fix is removal rather than patching
