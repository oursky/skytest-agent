...
import { isRunActiveStatus, type RunGroupSessionSummary } from '@/types';
import { isSchedulerTriggered } from '@/lib/test-runs/trigger-label';

interface SessionsResponse {
    groupName?: string;
    projectName?: string;
    data: RunGroupSessionSummary[];
    pagination?: { page: number; limit: number; total: number; totalPages: number };
}

export default function RunGroupHistoryPage({ params }: { params: Promise<{ groupId: string }> }) {
    ...
    const [sessions, setSessions] = useState<RunGroupSessionSummary[]>([]);
    ...
    const fetchSessions = useCallback(async () => {
        ...
        const response = await fetchWithAccessToken(getAccessToken, `/api/projects/${projectId}/run-groups/${groupId}/sessions?${query.toString()}`);
        if (response.ok) {
    ...
    return (
        <main className="min-h-screen bg-gray-50">
            <div className="mx-auto max-w-7xl px-8 py-8">
                <Breadcrumbs items={[
                    { label: projectName, href: projectId ? `/projects/${projectId}?tab=run-groups` : undefined },
                    { label: groupName },
                ]} />

                <div className="mb-8 flex items-center gap-3">
                    <h1 className="text-3xl font-bold text-gray-900">{t('runGroup.history.title')}</h1>
                </div>
                ...
                <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                    ...
                    {loaded && sessions.length === 0 ? (
                        <div className="p-16 text-center">
                            <h3 className="mb-2 text-lg font-semibold text-gray-900">{t('runGroup.history.empty.title')}</h3>
                            <p className="text-gray-500">{t('runGroup.history.empty.subtitle')}</p>
                        </div>
                    ) : (
                        <>
                                ...
                                {sessions.map((session) => (
                                    <div key={session.id} className="grid grid-cols-1 items-center gap-4 p-4 transition-colors hover:bg-gray-50 md:grid-cols-12">
                                        ...
                                        <div className="md:col-span-4">
                                            ...
                                            <div className="text-xs text-gray-400">{t('runGroup.caseCount', { count: session.memberCount })}</div>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-end md:col-span-3">
                                            <Link
                                                href={`/run-groups/runs/${session.id}?projectId=${projectId}`}
