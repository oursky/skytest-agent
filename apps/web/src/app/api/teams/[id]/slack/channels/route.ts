...
import { listConversations, type SlackConversationsPage } from '@/lib/integrations/slack/client';

const logger = createLogger('api:teams:slack-channels');
...
export async function GET(
    ...
    try {
        ...
        const cacheKey = `channels:${guard.teamId}:${cursor ?? ''}:${query ?? ''}:${limit}`;
        const cached = getLookupCacheValue<SlackConversationsPage>(cacheKey);
        if (cached) {
        ...
        logger.warn('Failed to list Slack channels', {
